...
class ModelBsmMC(ModelABC):
    ...
    def price(self, strike, spot, texp=1.0, cp=1):
        ...
        sigma_t = self.sigma * vol_path
        sigma_t = sigma_t[:-1]
         
        n_dt = int(np.ceil(texp / self.dt))
        tobs = np.arange(1, n_dt + 1) / n_dt * texp
        assert texp == tobs[-1]
        X_t = np.random.standard_normal((n_dt, self.n_path))
        Z_t = np.random.standard_normal((n_dt, self.n_path))
        W_t = self.rho * Z_t + np.sqrt(1 - self.rho**2) * X_t
        accumulation1 = np.sqrt(self.dt) * np.cumsum(sigma_t * W_t, axis = 0)
        accumulation2 = -1/2 * self.dt * np.cumsum(np.square(sigma_t), axis = 0)
        S_t = spot * np.exp(accumulation1 + accumulation2)
...
class ModelNormMC(ModelBsmMC):
    ...
    def price(self, strike, spot, texp=1.0, cp=1):
        ...
        accumulation1 = np.sqrt(self.dt) * np.cumsum(W_t * sigma_t, axis = 0)
        S_t = spot + accumulation1
